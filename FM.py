...
def parse_args():
    ...
    parser.add_argument('--noise', type=float, default=0.0, help='learning rate decay ratio')
...
class FM(BaseEstimator, TransformerMixin):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    ...
                    self.loss = tf.contrib.losses.log_loss(self.out, self.train_labels, epsilon=1e-07,
                                                           scope=None)
            self.reg_loss = tf.nn.l2_loss(self.weights['feature_embeddings'])

            global_step = tf.Variable(0, trainable=False)
            self.learning_rate1 = tf.train.exponential_decay(self.learning_rate, global_step,
                                                             len(np.array(
                                                                 self.data.Train_data['X'])) / self.batch_size,
                                                             self.learning_decay,
                                                             staircase=True)
            # Optimize
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            print(self.sess.run(self.reg_loss))
            t2 = time()
...
if __name__ == '__main__':
    ...
    print("Best train = %.4f, valid = %.4f, test = %.4f [%.1f s]"
          % (best_mse, best_auc, best_logloss,
             time() - t1))
    np.save("results/train_train_%s_FM.npy" % args.dataset, train)
    np.save("results/train_valid_%s_FM.npy" % args.dataset, valid)
    np.save("results/train_test_%s_FM.npy" % args.dataset, test)
    np.save("results/best_%s_FM.npy" % args.dataset, model.best_prediction)

    with open('results/%s_FM.txt' % args.dataset, 'a') as f:
        f.write("%f %f %f\n" % (best_mse, best_auc, best_logloss))
