...
if __name__ == '__main__':
    ...
    print("Best train = %.4f, valid = %.4f, test = %.4f [%.1f s]"
          % (best_mse, best_auc, best_logloss,
             time() - t1))
    np.save("results/train_train_%s_MixNeuralFM.npy" % args.dataset, train)
    np.save("results/train_valid_%s_MixNeuralFM.npy" % args.dataset, valid)
    np.save("results/train_test_%s_MixNeuralFM.npy" % args.dataset, test)
    np.save("results/best_%s_MixNeuralFM.npy" % args.dataset, model.best_prediction)

    with open('results/%s_MixNeuralFM.txt' % args.dataset, 'a') as f:
        f.write("%f %f %f\n" % (best_mse, best_auc, best_logloss))
