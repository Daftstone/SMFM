...
def parse_args():
    ...
    parser.add_argument('--noise', type=float, default=0., help='learning rate decay ratio')
...
class FM(BaseEstimator, TransformerMixin):
    ...
    def mix_all(self, data, error):
        ...
        one_index = np.random.permutation(len(X))[:int(len(X) * ratio)]
        one_index1 = np.random.choice(one_index, len(Y))
        X1 = X[one_index]
        Y1 = Y[one_index]
        W1 = W[one_index]
        rand_index = np.random.choice(one_index, int(len(Y) * ratio))
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        if self.verbose > 0:
            ...
            self.test_log.append(self.evaluate(Test_data, True))
    ...
    def evaluate(self, data, flag=False):  # evaluate the results for an input set
        num_example = len(data['Y'])
        X = np.array(data['X'])
        Y = np.array(data['Y'])[:, None]
        weights = data['weights']
        if (flag == True):
            weights = weights + (np.random.rand(weights.shape[0], weights.shape[1]) * 2. - 1.) * args.noise
...
if __name__ == '__main__':
    ...
    print("Best train = %.4f, valid = %.4f, test = %.4f [%.1f s]"
          % (best_mse, best_auc, best_logloss,
             time() - t1))
    np.save("results/train_train_%s_MixFM.npy" % args.dataset, train)
    np.save("results/train_valid_%s_MixFM.npy" % args.dataset, valid)
    np.save("results/train_test_%s_MixFM.npy" % args.dataset, test)
    np.save("results/best_%s_MixFM.npy" % args.dataset, model.best_prediction)

    with open('results/%s_MixFM.txt' % args.dataset, 'a') as f:
        f.write("%f %f %f\n" % (best_mse, best_auc, best_logloss))
