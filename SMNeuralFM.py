...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # self.gradient = tf.gradients(tf.contrib.losses.log_loss(self.out, self.train_labels, epsilon=1e-07, scope=None),
            #                              self.feature_weights)
            self.gradient = tf.gradients(self.loss, self.feature_weights)
    ...
    def evaluate_data1(self, data):  # evaluate the results for an input set
        ...
        for i in range((len(X) - 1) // batch_size + 1):
            ...
            cur_y = Y[i * batch_size:np.minimum(i * batch_size + batch_size, len(X))]
            cur_w = weights[i * batch_size:np.minimum(i * batch_size + batch_size, len(X))]
            feed_dict = {self.train_features: cur_x, self.train_labels: cur_y, self.dropout_keep: self.keep_prob,
                         self.feature_weights: cur_w,
                         self.train_phase: False}
...
if __name__ == '__main__':
    ...
    print("Best train = %.4f, valid = %.4f, test = %.4f [%.1f s]"
          % (best_mse, best_auc, best_logloss,
             time() - t1))
    np.save("results/train_train_%s_AdvNeuralFM.npy" % args.dataset, train)
    np.save("results/train_valid_%s_AdvNeuralFM.npy" % args.dataset, valid)
    np.save("results/train_test_%s_AdvNeuralFM.npy" % args.dataset, test)
    np.save("results/best_%s_AdvNeuralFM.npy" % args.dataset, model.best_prediction)

    with open('results/%s_AdvNeuralFM.txt' % args.dataset, 'a') as f:
        f.write("%f %f %f\n" % (best_mse, best_auc, best_logloss))
