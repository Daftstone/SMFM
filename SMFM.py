...
class FM(BaseEstimator, TransformerMixin):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.loss_type == 'square_loss':
                ...
                if self.lamda_bilinear > 0:
                    ...
                    self.loss = tf.contrib.losses.log_loss(self.out, self.train_labels, epsilon=1e-07,
                                                           scope=None)
            # self.gradient = tf.gradients(tf.contrib.losses.log_loss(self.out, self.train_labels, epsilon=1e-07,
            #                                                         scope=None), self.feature_weights)
            self.gradient = tf.gradients(self.loss,
    ...
    def mix_all(self, data, epoch):
        ...
        one_index = np.concatenate([index for i in range(ratio)])
        X1 = X[one_index]
        Y1 = Y[one_index]
        W1 = W[one_index]
        rand_index = np.random.choice(index, int(len(X) * ratio))
        lam = np.random.beta(0.5, 0.5, (len(Y1)))
        lam = np.abs(lam - 0.5) + 0.5
        Y1 = Y1 * lam + Y[rand_index] * (1 - lam)
        lam = np.repeat(lam[:, None], X1.shape[1], axis=1)
        X1 = np.concatenate([X1, X[rand_index]], axis=1)
        weights1 = np.concatenate([W1 * lam, W[rand_index] * (1 - lam)], axis=1)

        error = self.evaluate_data1({'X': X1, 'Y': Y1, 'weights': weights1})
        # error = self.evaluate_data2({'X': X1, 'Y': Y1, 'weights': weights1})
        # np.save("results/loss_error_%s.npy" % args.dataset, np.concatenate([loss[:, None], error[:, None]], axis=1))
        error = np.reshape(error, (ratio, len(X)))
        error_index = np.argmax(error, axis=0)
        sort_index = error_index * len(X) + index

        # sort_index = np.argsort(-error)[:len(X)]
        X1 = X1[sort_index]
        Y1 = Y1[sort_index]
        ...
        X0 = X[zero_index]
        Y0 = Y[zero_index]
        rand_index = np.random.permutation(len(Y0))
        lam = np.ones((len(Y0)))
        Y0 = Y0 * lam + Y0[rand_index] * (1 - lam)
        lam = np.repeat(lam[:, None], X0.shape[1], axis=1)
        X0 = np.concatenate([X0, X0[rand_index]], axis=1)
        weights0 = np.concatenate([W[zero_index] * lam, W[rand_index] * (1 - lam)], axis=1)

        X = np.concatenate([X1, X0], axis=0)
        Y = np.concatenate([Y1, Y0], axis=0)
        weights = np.concatenate([weights1, weights0], axis=0)
        return X, Y, weights
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            t1 = time()
            X, Y, W = self.mix_all(Train_data, epoch)
    ...
    def evaluate_data1(self, data):  # evaluate the results for an input set
        num_example = len(data['Y'])
        X = np.array(data['X'])
        Y = np.array(data['Y'])[:, None]
        weights = data['weights']
        ...
        for i in range((len(X) - 1) // batch_size + 1):
            ...
            feed_dict = {self.train_features: cur_x, self.train_labels: cur_y, self.dropout_keep: 1.0,
                         self.feature_weights: cur_w,
                         self.train_phase: False}
            predictions = self.sess.run(self.gradient, feed_dict=feed_dict)[0]
            y_pred[i * batch_size:np.minimum(i * batch_size + batch_size, len(X))] = np.sum(
                predictions, axis=1)
        return y_pred

    def evaluate_data2(self, data):  # evaluate the results for an input set
        num_example = len(data['Y'])
        X = np.array(data['X'])
        Y = np.array(data['Y'])[:, None]
        weights = data['weights']
        ...
        for i in range((len(X) - 1) // batch_size + 1):
            ...
            feed_dict = {self.train_features: cur_x, self.train_labels: cur_y, self.dropout_keep: 1.0,
                         self.feature_weights: cur_w,
                         self.train_phase: False}
            predictions = self.sess.run(self.loss, feed_dict=feed_dict)
            y_pred[i * batch_size:np.minimum(i * batch_size + batch_size, len(X))] = predictions
        return y_pred
...
if __name__ == '__main__':
    ...
    print("Best train = %.4f, valid = %.4f, test = %.4f [%.1f s]"
          % (best_mse, best_auc, best_logloss,
             time() - t1))
    np.save("results/train_train_%s_AdvFM.npy" % args.dataset, train)
    np.save("results/train_valid_%s_AdvFM.npy" % args.dataset, valid)
    np.save("results/train_test_%s_AdvFM.npy" % args.dataset, test)
    np.save("results/best_%s_AdvFM.npy" % args.dataset, model.best_prediction)

    with open('results/%s_AdvFM.txt' % args.dataset, 'a') as f:
        f.write("%f %f %f\n" % (best_mse, best_auc, best_logloss))
